package com.pendragon.blueconnect.bluetoothcontroller;
 ...
 */
public class ChatBluetoothService {
     ...
     *
     */
    public ChatBluetoothService(Context context, Handler handler) {
    ...
    private synchronized void estableceEstado(int estado) {
        ...
        miHandler.obtainMessage(Constants.MESSAGE_STATE_CHANGE, estado, -1).sendToTarget();
    }
    ...
    public synchronized void conectado(BluetoothSocket socket, BluetoothDevice
        ...
        Message msg = miHandler.obtainMessage(Constants.MESSAGE_DEVICE_NAME);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.DEVICE_NAME, dispositivo.getName());
        msg.setData(bundle);
    ...
    private void conexionFallida() {
        ...
        Message msg = miHandler.obtainMessage(Constants.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.TOAST, "No ha sido posible conectar con el dispositivo");
        msg.setData(bundle);
        miHandler.sendMessage(msg);

        // Inicia el servicio reseteando el modo de escucha.
       ChatBluetoothService.this.start();
    }
    ...
    private void conexionPerdida() {
        ...
        Message msg = miHandler.obtainMessage(Constants.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        bundle.putString(Constants.TOAST, "Se ha perdido la conexion con el dispositivo");
        msg.setData(bundle);
        miHandler.sendMessage(msg);
    ...
    private class AceptaHilo extends Thread {
        ...
        public void run() {
            ...
            while (miEstado != STATE_CONNECTED) {
                ...
                if (socket != null) {
                    synchronized (ChatBluetoothService.this) {
                        switch (miEstado) {
                            case STATE_LISTEN:
    ...
    private class HiloConectado extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    miHandler.obtainMessage(Constants.MESSAGE_READ, bytes, -1, buffer)
                            .sendToTarget();
                } catch (IOException e) {
        ...
        public void write(byte[] buffer) {
            try {
                ...
                miHandler.obtainMessage(Constants.MESSAGE_WRITE, -1, -1, buffer)
                        .sendToTarget();
            } catch (IOException e) {
